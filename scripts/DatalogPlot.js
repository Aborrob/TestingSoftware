$(document).ready(function () {
	...
	function decodeCSV(CSVdata) {//CSVdata = data of dataLog
		...
		var dataSet = [];//initialising dataSet to store data from dataArray
		// for (var x = 0; x < 4; x++) { // only first four column are shown on the graph (after the record and date and time columns)
		for (var x = 0; x < 2; x++) {
			if (x == 0) {
				dataSet[x] =
					{
						// label: CSVheader[x],
						label: CSVheader[1],//Bus voltage header as first time series
						data: dataArray[1],
				...
				dataSet[x] =
					{
						// label: CSVheader[x],
						label: CSVheader[chosenChart],//taken from the input field
						data: dataArray[chosenChart],
						yaxis: 2,
						lines: { show: true, fill: false, steps: false },
						points: { show: false }
					};
